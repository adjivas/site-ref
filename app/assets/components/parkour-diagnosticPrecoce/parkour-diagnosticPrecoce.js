    ...
    addNode: function(obj, coeff, layer, i) {
        ...
        img1.onload = function () {
            var valid = new Konva.Image({
                x: node.x() - coeff - coeff / 10,
                y: node.y() + coeff / 2,
                width: coeff / 2 + coeff / 12,
                height: coeff / 2 + coeff / 12,
                image: img1,
                id: "success" + String(i),
                visible: false
            });
        ...
        img2.onload = function () {
            var fail = new Konva.Image({
                x: node.x() + coeff + coeff / 10 - coeff / 2 - coeff / 12,
                y: node.y() + coeff / 2,
                width: coeff / 2 + coeff / 12,
                height: coeff / 2 + coeff / 12,
                image: img2,
        ...
        img3.onload = function () {
            var reset = new Konva.Image({
                x: node.x() - (coeff / 2 + coeff / 12) / 2,
                y: node.y() + coeff / 2 + coeff / 6,
                width: coeff / 2 + coeff / 12,
                height: coeff / 2 + coeff / 12,
                image: img3,
                id: "return" + String(i),
